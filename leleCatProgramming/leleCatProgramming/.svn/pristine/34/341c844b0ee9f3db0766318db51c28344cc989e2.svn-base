...
<email:html emailType="发件箱" title="发件箱" ctx="${ctx}">
	<div class="col-sm-3">
	...
	<div class="col-sm-9 animated fadeInRight">
		<div class="mail-box-header">
			...
			<email:tools ctx="${ctx}" emailType="发件箱" />
		</div>
		<div class="mail-box">
						...
						<c:forEach items="${requestScope.emails}" var="email">
							<tr class="unread readSign">
								...
								<td class="mail-ontact">
									<a href="${ctx}/admin/email/seeDetail/detail?emailId=${email.key.emailId}&detailType=发件箱">${email.key.receiver.name}</a>
								</td>
								<td class="mail-subject">
									<span class="readState label label-warning pull-right">${email.value}</span>
									<a href="${ctx }/admin/email/seeDetail/detail?emailId=${email.key.emailId}&detailType=发件箱">${email.key.emailTheme}</a>
								</td>
								<td align="center">
										${email.key.time}
								</td>
